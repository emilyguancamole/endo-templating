...
def merge_configs(base: dict, addon: dict) -> dict:
    ...
    Deep merge two configuration dictionaries by:
        ...
        - system_instructions: addon replaces base if present
    ...
    for key, value in addon.items():
        if key == '$extends':
            ...
            if 'field_groups' not in result:
                ...
                # Merge field_groups w/ module-wide insert_after
                insert_after_hint = addon.get('meta', {}).get('insert_after')
                result['field_groups'] = merge_field_groups_with_position(result['field_groups'], value, insert_after_hint)
...
def merge_field_groups_with_position(base_groups: dict, addon_groups: dict, insert_after: str = None) -> dict:
    ...
    for name, cfg in addon_groups.items():
        if name not in merged:
            new_groups.append((name, deepcopy(cfg)))
    if not new_groups:
        return merged

    result = OrderedDict()
    # Iterate existing base keys and inject new groups after anchor
...
def generate_pydantic_model(config: dict, model_name: str = "ERCPData") -> str:
    ...
    # Add validator for integer fields to handle "unknown" string
    if integer_fields:
        lines.append("")
    ...
    # Add validator for float fields to handle "unknown" string
    if float_fields:
        lines.append("")
...
def generate_single(fields_yaml_path: str, proc_type: str = None):
    ...
    # Write generated models to the models directory
